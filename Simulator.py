...
def v_max (veh,R, Fy_max=0):
    def func(v):
        A = veh.mu_lat_0 * R/veh.m_total_kg*(veh.fz_ref_N)**veh.load_sens_exp
        W = veh.m_total_kg*9.81
        D = 0.5 * veh.rho_air_kgpm3 * veh.ClA
        return (A*(W+D*v**2)**(1-veh.load_sens_exp))-v**2
    sol = root_scalar(func, bracket=[0, 1e3], method='brentq')
    return sol.root
    # if np.any(Fy_max < EPS):
    #     print(Fy_max)
    # Front_vmax = np.sqrt(R*np.sum(Fy_max[:2])/veh.m_total_kg)
    # Rear_vmax = np.sqrt(R*np.sum(Fy_max[2:4])/veh.m_total_kg)
    # v = min(Front_vmax, Rear_vmax)
    # return v
...
def SimulateSegment(state, seg, veh, motor):
    ...
    v, s, t, E_Wh, idx, ax,ay = float(state.v), 0.0, 0.0, 0.0, 0, state.ax, state.ay
    ...
    while s < seg.L and idx < max_steps:
        ...
        if omega > motor.rpm_max:
            ax = 0.0
        if seg.R > EPS:
            if v > v_max(veh, seg.R, Fy_max):
                v = v_max(veh, seg.R, Fy_max)
            ay = v**2 / seg.R
        else:
            ay = 0.0
...
def SimulateLap(track, veh, motor):
    ...
    for k in range(1, track.shape[0]):
        seg = types.SimpleNamespace(R=np.abs(np.float64(track[k][1])), L=np.float64(track[k][2]), Left = np.float64(track[k][1]) > 0,Rnext = np.abs(np.float64(track[min(k+1, len(track)-1)][1])))
